<?php
    class User {
        ...
        public function add_article($title,$content,$article_image) {
            global $db;
            $title = $db->escape_string($title); //JAUTAJUMS
            $sql = "UPDATE users SET article_title='$title' , article_content='$content',article_image='$article_image' WHERE email='$this->email'";
            $db->query($sql);
